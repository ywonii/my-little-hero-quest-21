...
const supabaseKey =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inh1Zm5laWtwdmFrZ29tc25jcXNwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTUzMjkyOTUsImV4cCI6MjA3MDkwNTI5NX0.klkp0MzI6ZnEiVuW8tgydZNzszJ_NYJTOzmBWAgUQ20";
...
serve(async (req) => {
  ...
  try {
        ...
        messages: [
          {
            ...
            1개의 상황을 만들고, 그 상황에 대해 3가지 난이도 버전을 제공하세요.
        ...
        messages: [
          {
            role: "system",
            content:
              '주어진 문제 상황을 바탕으로 아이들이 이해하기 쉬운 테마 이름을 만들어주세요. 15글자 이내의 간단하고 친근한 제목으로 만드세요. 예: "친구와 갈등 해결하기", "학교에서 예의 지키기" 등. 테마 이름만 반환하세요.',
          },
          {
            role: "user",
    ...
    for (const difficulty of difficulties) {
      console.log(`Saving ${difficulty.key} scenario with difficulty_level: ${difficulty.level}`);
      ...
      console.log(`Saved scenario with id: ${scenarioData.id}, difficulty_level: ${scenarioData.difficulty_level}`);

      // 선택지 저장
      for (let i = 0; i < difficulty.data.options.length; i++) {
        const { error: optionError } = await supabase.from("scenario_options").insert([
          {
